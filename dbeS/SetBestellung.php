...
if (auth())
{
	...
	if (intval($_POST["action"]) == 6 && intval($_POST['KeyBestellung']))
	{
		...
		//hole einstellungen
		$cur_query = eS_execute_query("select StatusVersendet from ".DB_PREFIX."eazysales_einstellungen");
		$einstellungen = mysql_fetch_object($cur_query);
		
		//setze status der Bestellung
		if ($einstellungen->StatusVersendet>0 && $_POST["VersandDatum"])
		{
			eS_execute_query("update ".DB_PREFIX."orders set orders_status=".$einstellungen->StatusVersendet." where orders_id=".intval($_POST['KeyBestellung']));
			//füge history hinzu
			...
			$liefer_query = eS_execute_query("select delivery_postcode from ".DB_PREFIX."orders where orders_id=".intval($_POST['KeyBestellung'])); 
			...
			{ 
				eS_execute_query("insert into ".DB_PREFIX."orders_status_history (orders_id, orders_status_id, date_added, comments, customer_notified) values(".intval($_POST['KeyBestellung']).", ".$einstellungen->StatusVersendet.", now(), \"".$kommentar_txt."\", 1)"); 
				//mail aus XTC rausschicken 
				sende_xtc_mail(intval($_POST['KeyBestellung']),$kommentar_txt,$kommentar_html); 
			} 
			else 
			{ 
				eS_execute_query("insert into ".DB_PREFIX."orders_status_history (orders_id, orders_status_id, date_added, comments) values(".intval($_POST['KeyBestellung']).", ".$einstellungen->StatusVersendet.", now(), \"".$kommentar_txt."\")"); 
			}			
		}
 	}
	...
	if (intval($_POST["action"]) == 5 && intval($_POST['KeyBestellung']))
	{
		...
		//hole einstellungen
		$cur_query = eS_execute_query("select StatusAbgeholt from ".DB_PREFIX."eazysales_einstellungen");
		$einstellungen = mysql_fetch_object($cur_query);
		
		//setze status der Bestellung
		if ($einstellungen->StatusAbgeholt>0)
		{
			eS_execute_query("update ".DB_PREFIX."orders set orders_status=".$einstellungen->StatusAbgeholt." where orders_id=".intval($_POST['KeyBestellung']));
			//füge history hinzu
			$kommentar = "Erfolgreich in JTL-Wawi übernommen";
			eS_execute_query("insert into ".DB_PREFIX."orders_status_history (orders_id, orders_status_id, date_added, comments) values(".intval($_POST['KeyBestellung']).", ".$einstellungen->StatusAbgeholt.", now(), \"".$kommentar."\")");
		}
		
		//setze bestellung auf abgeholt
		eS_execute_query("insert into ".DB_PREFIX."eazysales_sentorders (orders_id, dGesendet) values (".intval($_POST['KeyBestellung']).",now())");
	}
}
...
function sende_xtc_mail($KeyBestellung, $kommentar_txt, $kommentar_html)
{
	...
	require_once (DIR_FS_CATALOG.DIR_WS_CLASSES . 'Smarty_3.0.7/Smarty.class.php');
