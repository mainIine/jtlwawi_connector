...
if (auth())
{
	...
	if ((intval($_POST["action"]) == 1 || intval($_POST["action"]) == 3) && intval($_POST['KeyKategorie'])>0)
	{
		...
		if ($Kategorie->kOberKategorie>0)
		{
			...
			if (!$categories_id_oberkat)
			{
				...
				eS_execute_query("insert into ".DB_PREFIX."categories_description (categories_id, language_id) values (".$categories_id_oberkat_arr[0].",$einstellungen->languages_id)");
				...
				if (is_array($sonstigeSprachen))
				{
					...
					{
						eS_execute_query("insert into ".DB_PREFIX."categories_description (categories_id, language_id) values (".$categories_id_oberkat_arr[0].",$sonstigeSprache)");
					}
				}
			}
			else 
		...
		{
			//update
			eS_execute_query("update ".DB_PREFIX."categories set parent_id=$Kategorie->parent_id, categories_status=1, sort_order=$Kategorie->nSort where categories_id=".$categories_id);
			eS_execute_query("update ".DB_PREFIX."categories_description set categories_name=\"$Kategorie->cName\", categories_description=\"$Kategorie->cBeschreibung\" where categories_id=".$categories_id." and language_id=".$einstellungen->languages_id);
		}
		else 
		{
			//insert
			eS_execute_query("insert into ".DB_PREFIX."categories (parent_id, categories_status, categories_template, listing_template, products_sorting, products_sorting2, date_added, sort_order) values ($Kategorie->parent_id,1,\"".$einstellungen->cat_category_template."\",\"".$einstellungen->cat_listing_template."\",\"".$einstellungen->cat_sorting."\",\"".$einstellungen->cat_sorting2."\",now(),$Kategorie->nSort)");
			$query = eS_execute_query("select LAST_INSERT_ID()");
			$categories_id_arr = mysql_fetch_row($query);
			eS_execute_query("insert into ".DB_PREFIX."categories_description (categories_id, language_id, categories_name, categories_description) values (".$categories_id_arr[0].",$einstellungen->languages_id, \"$Kategorie->cName\", \"$Kategorie->cBeschreibung\")");
			...
			if (is_array($sonstigeSprachen))
			{
				foreach ($sonstigeSprachen as $sonstigeSprache)
				{
					...
					eS_execute_query("insert into ".DB_PREFIX."categories_description (categories_id, language_id, categories_name, categories_description) values (".$categories_id_arr[0].",$sonstigeSprache, \"$Kategorie->cName\", \"$Kategorie->cBeschreibung\")");
				}
			}
		}
 	}	
	...
	if (intval($_POST["action"]) == 3 && intval($_POST['KeyKategorie'])>0)
	{
		...
		if ($cat>0)
			eS_execute_query("update ".DB_PREFIX."categories set categories_status=0 where categories_id=".$cat);
	}
}
