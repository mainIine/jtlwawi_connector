...
if (auth())
{
	...
	//get currency
	$cur_query = eS_execute_query("select * from ".DB_PREFIX."currencies where currencies_id=".$einstellungen->currencies_id);
	...
	$cur_query = eS_execute_query("SELECT 
									...
									WHERE 
										em.products_id is NULL LIMIT 1");	
	if ($product_id = mysql_fetch_object($cur_query))
	{		
		//hole product		
		$product_query = eS_execute_query("select * from ".DB_PREFIX."products where products_id=".$product_id->products_id);
		if ($product = mysql_fetch_object($product_query))
		{	
			//hole beschreibung
			$product_desc_query = eS_execute_query("select * from ".DB_PREFIX."products_description where products_id=".$product_id->products_id." and language_id=".$einstellungen->languages_id." order by products_name");
			...
			{
				$vpe_query = eS_execute_query("select products_vpe_name from ".DB_PREFIX."products_vpe where language_id=".$GLOBALS['einstellungen']->languages_id." and products_vpe_id=".$product->products_vpe);		
...
{
	$bilderUrls="";
	$cur_query = eS_execute_query("select configuration_value from ".DB_PREFIX."configuration where configuration_key=\"MO_PICS\"");
	...
	{
		//hole bilder
		$cur_query = eS_execute_query("select * from ".DB_PREFIX."products_images where products_id=".$product->products_id." order by image_nr");
...
function get_attribute($product)
{
	...
	{
		//hole bezeichnung
		$cur_query = eS_execute_query("select shipping_status_name from ".DB_PREFIX."shipping_status where language_id=".$GLOBALS['einstellungen']->languages_id." and shipping_status_id=".$product->products_shippingtime);
...
function get_variationswerte($products_id,$options_id)
{
	...
	$cur_query = eS_execute_query("select * from ".DB_PREFIX."products_attributes where options_id=".$options_id." and products_id=".$products_id);
	while ($variation = mysql_fetch_object($cur_query))
	{
		if ($variation->options_values_id>0)
		{
			//hole Variationswertnamen etc.
			$opt_query = eS_execute_query("select products_options_values_name, products_options_values_id from ".DB_PREFIX."products_options_values where language_id=".$GLOBALS['einstellungen']->languages_id." and products_options_values_id=".$variation->options_values_id);
...
function get_variationen($products_id)
{
	...
	$cur_query = eS_execute_query("select * from ".DB_PREFIX."products_attributes where products_id=".$products_id." group by options_id");
	while ($variation = mysql_fetch_object($cur_query))
	{
		if ($variation->options_id>0)
		{
			//hole Variationsname etc.
			$opt_query = eS_execute_query("select products_options_name, products_options_id from ".DB_PREFIX."products_options where language_id=".$GLOBALS['einstellungen']->languages_id." and products_options_id=".$variation->options_id);
			$var_name = mysql_fetch_object($opt_query);
...
{	
	...
	if ($endKunden_arr[0]>=0 && strlen($endKunden_arr[0])>0)
	{
		...
		$cur_query = eS_execute_query("select * from ".DB_PREFIX."$personalOfferTable where quantity=1 and products_id=".$product->products_id);
...
function get_cats($products_id)
{
	$res ="";
	//get cat_id
	$glob_cat_query = eS_execute_query("select * from ".DB_PREFIX."products_to_categories where products_id=".$products_id);
	while ($act_cat = mysql_fetch_object($glob_cat_query))
	{
		...
		if ($cur_cat_id>0)
		{
			$cat_query = eS_execute_query("select * from ".DB_PREFIX."categories where categories_id=".$cur_cat_id);
		...
		for ($i=0;$i<$cnt;$i++)
		{
			...
			$cat_query = eS_execute_query("select * from ".DB_PREFIX."categories_description where categories_id=".$catId." and language_id=".$GLOBALS['einstellungen']->languages_id);
			$current_cat = mysql_fetch_object($cat_query);
			$cat_query = eS_execute_query("select categories_status, categories_image, sort_order from ".DB_PREFIX."categories where categories_id=".$catId);
...
//get hersteller
function getManufacturer($brand_id)
{
	if ($brand_id>0)
	{
		$manu_query = eS_execute_query("select * from ".DB_PREFIX."brand where brand_id=".$brand_id);
		$manu = mysql_fetch_object($manu_query);
		return ($manu->brand_name);	
	}
