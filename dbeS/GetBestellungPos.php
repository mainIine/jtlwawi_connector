...
if (auth())
{
	...
	{
		//glob einstellungen
		$cur_query = eS_execute_query("select versandMwst,tax_zone_id from ".DB_PREFIX."eazysales_einstellungen");
		$einstellungen = mysql_fetch_object($cur_query);
		$allowTax = 1;
		
		$return = 0;		
		//hole orderposes
		$cur_query = eS_execute_query("select * from ".DB_PREFIX."orders_products where orders_id=".intval($_POST['KeyBestellung'])." order by orders_products_id");
		while ($BestellungPos = mysql_fetch_object($cur_query))
		{
			...
			//hole etl aufpreise
			$aufpreis=0;
			$aufpreise_query = eS_execute_query("select options_values_price,price_prefix from ".DB_PREFIX."orders_products_attributes where orders_id=".$BestellungPos->orders_id." and orders_products_id=".$BestellungPos->orders_products_id." and options_values_price!=0");
		...
		$cur_query = eS_execute_query("select * from ".DB_PREFIX."orders_total where (class=\"ot_shipping\" OR class=\"ot_cod_fee\" OR class=\"ot_coupon\" OR class=\"ot_discount\" OR class=\"ot_orderdiscount\" OR class=\"ot_gv\" OR class=\"ot_loworderfee\" OR class=\"ot_ps_fee\" OR class=\"ot_payment\") and orders_id=".intval($_POST['KeyBestellung'])." order by sort_order");
			...
			if ($total_pos->class=="ot_shipping" || $total_pos->value!=0)
			{
				...
				switch ($total_pos->class)
				{
					case 'ot_shipping':
						...
						break;
					case 'ot_cod_fee':
						$tax_query = eS_execute_query("select configuration_value from ".DB_PREFIX."configuration where configuration_key=\"MODULE_ORDER_TOTAL_COD_FEE_TAX_CLASS\"");
						...
						break;
					case 'ot_coupon':
						$tax_query = eS_execute_query("select configuration_value from ".DB_PREFIX."configuration where configuration_key=\"MODULE_ORDER_TOTAL_COUPON_TAX_CLASS\"");
						$tax_class = mysql_fetch_object($tax_query);
						$steuersatz = get_tax($tax_class->configuration_value);
						$total_pos->value*=-1;
						if (!$allowTax)
						...
						break;
					case 'ot_gv':
						$tax_query = eS_execute_query("select configuration_value from ".DB_PREFIX."configuration where configuration_key=\"MODULE_ORDER_TOTAL_GV_TAX_CLASS\"");
						$tax_class = mysql_fetch_object($tax_query);
						$steuersatz = get_tax($tax_class->configuration_value);
						$total_pos->value*=-1;
						if (!$allowTax)
						...
						break;
					case 'ot_loworderfee':
						$tax_query = eS_execute_query("select configuration_value from ".DB_PREFIX."configuration where configuration_key=\"MODULE_ORDER_TOTAL_LOWORDERFEE_TAX_CLASS\"");
						$tax_class = mysql_fetch_object($tax_query);
						$steuersatz = get_tax($tax_class->configuration_value);
						if (!$allowTax)
						{
						...
						break;				
					case 'ot_ps_fee':
						$tax_query = eS_execute_query("select configuration_value from ".DB_PREFIX."configuration where configuration_key=\"MODULE_ORDER_TOTAL_PS_FEE_TAX_CLASS\"");
						$tax_class = mysql_fetch_object($tax_query);
						$steuersatz = get_tax($tax_class->configuration_value);
						if (!$allowTax)
						{
